...
const x = d3.scaleLinear()
  .domain([
    windowStart + VIEW_OFFSET,
    windowStart + VIEW_OFFSET + INITIAL_VIEW_YEARS
  ])
  .range([0, W]);
...
function renderAxes() {
  topAxisG.call(
    d3.axisTop(x)
      .ticks(10)
      .tickFormat(d3.format("d")) // force nice integer year labels
  );
  botAxisG.call(
    d3.axisBottom(x)
      .ticks(10)
...
function drawLines(filtered) {
  ...
  halos = halos.enter()
    ...
    .attr("class", "halo")
    .merge(halos)
    .attr("d", d => `M${x(d.startYear)},${yTop} L${x(d.endYear)},${yBot}`)
...
function drawEvents() {
  ...
  events.attr("transform", d => `translate(${x(d.year)},0)`);
...
svg.on("wheel", (e) => {
  ...
  // Equal sensitivity for up/down
  const SCROLL_SPEED = 0.001;   // make smaller for slower motion

  panDomain(dy * SCROLL_SPEED);
});
...
function panDomain(delta) {
  const [minYear, maxYear] = x.domain();
  const span = maxYear - minYear;

  // continuous movement
  let newMin = minYear + delta * span;
  let newMax = newMin + span;

  // strict clamps (so domain never becomes invalid)
  ...
  update();         // ← this redraws axes + lines continuously
}
...
function animateScrollToYear(targetYear, duration = 250) {
  ...
  const [startMin, startMax] = x.domain();
